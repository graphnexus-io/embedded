...
InputEvent character_event(char character)
{
    ...
    switch (character) {
        ...
        case 'U': return InputEvent::UNDO;
        case '[': return InputEvent::PAGE_PREVIOUS;
        case ']': return InputEvent::PAGE_NEXT;
        default: return InputEvent::NONE;
    }
}
...
InputEvent serial_input_poll()
{
    ...
    while (Serial.available() > 0) {
        ...
        previous_was_cr = false;
        const InputEvent event = character_event(character);
        if (event != InputEvent::NONE) {
            return event;
        }
    }
